...
def tensor_to_image(tensor, save_path):
    ...
    Parameters:
    tensor (torch.Tensor): A tensor with shape (3, 512, 512) normalized by a factor of 0.5.
    save_path (str): The path where the image will be saved.
    """
    # Ensure the tensor is on the CPU and convert it to a numpy array
    tensor = tensor.cpu().numpy()

    # Denormalize the tensor by multiplying by the inverse of the normalization factor
    tensor = tensor * 0.5

    # Convert the tensor from the range [-1, 1] to [0, 1]
    tensor = (tensor + 1) / 2

    # Convert the tensor to the range [0, 255] and change the data type to uint8
    tensor = (tensor * 255).astype(np.uint8)

    # Transpose the tensor from (3, 512, 512) to (512, 512, 3) for image representation
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--add_optical_flow", action="store_true", help="Max gradient norm.")
    parser.add_argument("--of_norm_factor", type=float, default=1, help="Weight decay to use.")
    parser.add_argument("--add_warped_image", action="store_true", help="Max gradient norm.")
...
def make_train_dataset(args, tokenizer, accelerator, phase="train"):
    ...
    optical_flow_transforms = transforms.Compose(
        [
            transforms.ToTensor(),
            # transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.CenterCrop(args.resolution),
    ...
    def preprocess_train(examples):
        ...
        if args.add_warped_image:
            for warped_image_path in warped_image_paths:
                ...
                examples["warped_image"].append(warped_image)
...
def main(args):
    ...
    validation_dataset = make_train_dataset(args, tokenizer, accelerator, phase="validation")
    ...
    controlnet, optimizer, train_dataloader, validation_dataloader, lr_scheduler = accelerator.prepare(
        controlnet, optimizer, train_dataloader, validation_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                optical_flow = batch["optical_flow"].to(dtype=weight_dtype) / args.of_norm_factor
                if args.add_warped_image:
                    warped_image = batch["warped_pixel_values"].to(dtype=weight_dtype)
                    merged_input = torch.cat([controlnet_image, optical_flow, warped_image], dim=1)
                else:
                    merged_input = torch.cat([controlnet_image, optical_flow], dim=1)

                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    controlnet_cond=merged_input,
                    return_dict=False,
                )
